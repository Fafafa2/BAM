...
def run(data,
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        # Run model
        # out, train_out = model(img, targets,augment=augment)  # inference and training outputs
        out, train_out = model(img, augment=augment)  # i
        # Allout, attention_loss = model((img), targets, augment=augment)
        # out, train_out = Allout
    ...
    #     FPS = 1000 / sum(t)
    #     LOGGER.info(f'FPS:{round(FPS, 3)}')fo(f'Speed: %.1fms pre-process, %.1fms inference, %.1fms NMS per image at shape {shape}' % t)
    if not training:
        shape = (batch_size, 3, imgsz, imgsz)
        LOGGER.info(f'Speed: %.1fms pre-process, %.1fms inference, %.1fms NMS per image at shape {shape}' % t)
        FPS = 1000 / sum(t)

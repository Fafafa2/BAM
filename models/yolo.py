...
from utils.plots import feature_visualization_all,colorslabel_visualization_clusters,dancolorslabel_visualization_clusters
...
class CLLABlock(nn.Module):
    def __init__(self, range=2, ch=256, ch1=128, ch2=256, out=0):
        ...
        self.det = nn.Conv2d(self.c_+1, out, 1)

    def forward(self, x1, x2):
        x1 = self.conv1(x1)
        x2 = self.conv2(x2)
        f = self.att(x1, x2)
        Attention=BAM(f,f.detach())
        newf = torch.cat((f, Attention), dim=1)
        return self.det(newf)
...
def BAM(feature_0, feature_1):

    batchsize, c, h, w = feature_0.shape
    feature_reshaped = feature_1.view(batchsize, c, h * w).transpose(1, 2)
    top_clusters = []
    batch_cluster_ids = []
    batch_predicted_ids = []
    for i in range(batchsize):
        cluster_ids_x, centers = kmeans(feature_reshaped[i, ...], num_clusters=5, distance='cosine', device=torch.device('cuda:0'), iter_limit=1)
        
        counts = torch.bincount(cluster_ids_x, minlength=centers.size(0))
        sorted_indices = torch.argsort(counts, descending=True)
    
        top_count_clusters = sorted_indices[:4]
        top_count_centers = centers[top_count_clusters] 
        top_clusters.append(top_count_centers.unsqueeze(0))
    top_clusters = torch.cat(top_clusters, dim=0)
    top_clusters_norm = torch.norm(top_clusters, dim=2, keepdim=True)
    top_clusters_normalized = top_clusters / top_clusters_norm

    
    feature_0 = feature_0.view(batchsize, c, h * w)
    feature_0_norm = torch.norm(feature_0, dim=1, keepdim=True)
    feature_0_normalized = feature_0 / feature_0_norm

    similarity_map = torch.bmm(top_clusters_normalized, feature_0_normalized)
    similarity_map = (similarity_map + 1) / 2
    similarity_map, _ = similarity_map.max(dim=1, keepdim=True)
    similarity_map = similarity_map.view(batchsize, 1, h, w)

    object_attention = 1 - similarity_map
...
class Model(nn.Module):
    ...
    def _forward_once(self, x, target=None,profile=False, visualize=False):
        y, dt = [], []  # outputs
        maps = {"2": [None, None], "17": [None, None], "20": [None, None], "23": [None, None]}
        finalDetectLayer = None

        for idx, m in enumerate(self.model):
            if isinstance(m, CLLADetect):
                finalDetectLayer = m
                continue
            ...
            x = m(x)  # run
            if str(idx) in maps.keys():
                maps[str(idx)][0] = x
            y.append(x if m.i in self.save else None)  # save output
            if visualize:
                feature_visualization_all(x, m.type, m.i, "pic1", save_dir=visualize)


        final_weighted_features = []
        for layer,feature in maps.items():
            if (layer=='2'or layer=='17'):
                weighted_features = feature[0]
            else:
                batchsize, c, h, ww = feature[0].shape
                juleiattention_map = BAM(feature[0],feature[0].detach())
                weighted_features = feature[0] * juleiattention_map
            final_weighted_features.append(weighted_features)
        x = finalDetectLayer(final_weighted_features)
